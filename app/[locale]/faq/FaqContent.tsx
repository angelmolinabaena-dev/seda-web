    ...
    <main id="main-content">
      ...
      <section className="px-6 md:px-12 lg:px-20 py-16 md:py-24">
        <div className="max-w-3xl mx-auto">
          ...
          {/* Both lists are ALWAYS in the DOM (SSR'd, crawlable) — the
              inactive one is hidden with the `hidden` utility (display:none),
              not conditionally unrendered. Previously only the active tab's
              array was ever mapped into JSX, so the 6 owner questions (the
              highest-value B2B content on the site) never reached the DOM
              at all — only the guest tab's HTML shipped on first paint, and
              switching tabs was a pure client-state toggle with no new
              content. Mirrors the `hidden`-class tab pattern already used
              in `app/[locale]/propietarios/page.tsx`. */}
          <ul className={`border-t border-border ${tab === "guests" ? "" : "hidden"}`}>
            {FAQ_GUESTS.map((item) => (
              <AccordionItem key={item.q} q={item.q} a={item.a} />
            ))}
          </ul>
          <ul className={`border-t border-border ${tab === "owners" ? "" : "hidden"}`}>
            {FAQ_OWNERS.map((item) => (
              <AccordionItem key={item.q} q={item.q} a={item.a} />
            ))}
          </ul>
